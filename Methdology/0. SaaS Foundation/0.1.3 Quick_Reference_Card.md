...
## 11. 금액 저장 및 처리 규칙 (결정 25)

> 근거: Framework 결정 25 — "금액은 NUMERIC(19,4)로 저장, 원 단위 절사, float 사용 금지"  
> ⚠️ 위반 시: 부동소수점 오차로 공공기관 정산 오류 발생 → 치명적
...
def truncate_to_won(amount: Decimal) -> Decimal:
    return amount.quantize(Decimal("1"), rounding=ROUND_DOWN)  # 원 단위 절사

def calc_vat(amount: Decimal) -> Decimal:
    return truncate_to_won(amount * Decimal("0.1"))             # VAT 10%

def calc_total(amount: Decimal) -> Decimal:
    return amount + calc_vat(amount)

# ❌ 금지
# amount * 0.1        → float 연산, 정밀도 손실
# round(amount, 0)    → round() 금지, quantize 사용
```
...
class InvoiceResponse(BaseModel):
    ...
    total:  Decimal
    model_config = {
        "json_encoders": { Decimal: lambda v: str(v) }  # JSON 시 문자열 변환
    }
# ✅ 응답: { "amount": "100000.0000" }
# ❌ 금지: { "amount": 100000.0 }  → JS float 오차
```

### Frontend — 금액 타입 및 포맷터

```typescript
// types/money.ts
export type MoneyString = string    // API 수신용 ("100000.0000")

// utils/money.ts
export function formatKRW(value: MoneyString | number): string {
  const num = typeof value === 'string' ? parseFloat(value) : value
  return new Intl.NumberFormat('ko-KR', {
    style: 'currency', currency: 'KRW',
    minimumFractionDigits: 0, maximumFractionDigits: 0,
  }).format(Math.floor(num))  // 원 단위 절사
}
// ✅ formatKRW("100000.0000") → "₩100,000"
```

```tsx
// 금액 표시
<span className="font-mono tabular-nums">{formatKRW(amount)}</span>
...
| 금지 패턴 | 이유 | 올바른 대안 |
|-----------|------|------------|
| `Float` / `float` DB 컬럼 | 부동소수점 오차 | `Numeric(19, 4)` |
| `amount * 0.1` float 연산 | 정밀도 손실 | `Decimal("0.1")` |
| `round(amount, 0)` | 반올림 방식 불명확 | `quantize(ROUND_DOWN)` |
| `parseFloat()` 결과 저장 | JS float 오차 | 문자열 그대로 API 전송 |
| `Integer` 금액 컬럼 | 소수점 지원 불가 | `Numeric(19, 4)` |
...
| "에러 코드가 뭐지?" | 📙 이 문서 — Section 3 |
| "API URL 형식이 뭐지?" | 📙 이 문서 — Section 2 |
| "금액은 어떻게 처리하나?" | 📙 이 문서 — Section 11 |
| "UI 컴포넌트 기준은?" | 📓 UI/UX Design Guide (doc5) — Section 4~5 |
| "AI Agent Boilerplate는?" | 📒 Boilerplate Guide (doc4) — Section 4 |
