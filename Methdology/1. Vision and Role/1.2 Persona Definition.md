...
- WBS Task
- 1.2.1 주요 Persona 정의 (Primary / Secondary User)
- 1.2.2 Usage Scenario 작성 (Happy Path / Edge Case)
- 1.2.3 Context Requirements 정의
...
Problem Framing(1.1)의 'Who' 섹션에서 식별된 5개 이해관계자 그룹을 기반으로 Persona를 구체화하였으며, 각 Persona의 목표, Pain Point, 행동 패턴, AI 활용 방식을 정의한다.
...
|-------------|----------------------------------------------------------------------------------------|
| Pain Point  | 프로젝트마다 아키텍처가 달라 유지보수 비용 증가 / AI 의사결정 기록 체계 없어 감사 대응 시 수작업 / 보안/컴플라이언스 기준을 매번 새로 정의해야 함 |
...
| AI 사용 목적 | 감사 로그 자동 집계 및 AI 의사결정 추적 보고서 생성  |
|----------|----------------------------------|
| 기대 행동    | 감사 요청 시 기간/범위 입력만으로 표준 보고서 즉시 출력 |
| 신뢰 수준    | AI 생성 보고서를 검토 후 최종 제출 (L1 수준)    |
| 우려 사항    | AI가 로그를 누락하거나 의사결정 근거를 잘못 해석하는 것 |
...
|-------------|----------------------------------------------------------------------------------------------------------|
| Pain Point  | 프로젝트마다 권한관리, 로깅, API 구조를 처음부터 만들어야 함 / 표준 없어서 팀원마다 코딩 스타일이 달라 코드 리뷰 부담 / AI 거버넌스 요구사항이 프로젝트 중간에 추가되어 재작업 |
...
|-------------|-----------------------------------------------------------------------------------|
| Pain Point  | AI가 어떤 근거로 추천했는지 설명할 수 없음 / 감사 준비 시 로그 수집에 평균 4주 이상 소요 / 시스템마다 로그 형식이 달라 통합 분석 불가 |
...
|-------------|---------------------------------------------------------------------------------------------|
| Pain Point  | 모델 승인 절차가 팀마다 달라 평균 6~12주 소요 / Drift 감지 체계가 없어 성능 저하를 사후에 발견 / 책임 소재가 불명확해 장애 발생 시 원인 파악 지연 |
| Motivation  | 표준화된 Release Gate와 자동 Drift 탐지로 운영 안정성 확보                                                   |
| Frustration | 배포 후에 문제가 생겨도 어디서 잘못됐는지 추적이 안 된다                                                            |

AI 활용 방식

| AI 사용 목적 | 모델 배포·모니터링 현황 자동 수집 및 Drift 이상 징후 탐지    |
|----------|-----------------------------------------|
| 기대 행동    | 배포 전 체크리스트 자동 생성, KPI 임계값 초과 시 즉시 알림    |
| 신뢰 수준    | AI 탐지 결과를 기반으로 최종 배포 승인은 본인이 결정 (L1 수준) |
| 우려 사항    | AI가 Drift를 놓치거나 장애 원인 추적 경로를 잘못 제시하는 것  |
...
|-------------|-----------------------------------------------------------------------------------------------|
| Pain Point  | AI 통제 체계가 불투명해 규제 준수 여부 확인 어려움 / 기관마다 AI 시스템 구조가 달라 일관된 기준 적용 불가 / 기술 용어 중심 문서로 정책 관점 검토가 어려움 |
| Motivation  | 표준 프레임워크 채택 시 규제 준수 여부를 구조적으로 검증 가능                                                           |
| Frustration | 잘 만들었다고 하는데 실제로 어떻게 통제되는지 설명을 못 한다                                                            |

AI 활용 방식

| AI 사용 목적 | AI 거버넌스 준수 현황 자동 보고서 생성 및 규제 기준 대조 분석       |
|----------|---------------------------------------------|
| 기대 행동    | 정책 담당자가 이해할 수 있는 비기술 언어로 AI 통제 구조 설명 출력     |
| 신뢰 수준    | AI 보고서를 검토 후 규제 준수 여부 최종 판단은 본인이 수행 (L1 수준) |
| 우려 사항    | AI가 규제 기준을 잘못 해석하거나 통제 구조를 불투명하게 설명하는 것     |
...
## 3.3 Scenario 3 — 비 표준 요구사항 예외 처리 (Edge Case)
